...
fn main() {
    ...
    if args[1] == "--server" {
        println!("===INITATING SERVER MODE===");
        let listener = TcpListener::bind("0.0.0.0:8888").expect("Cannot bind");
        for stream in listener.incoming() {
            match stream {
                Err(e) => eprintln!("‼️ Failed: {}", e),
                Ok(stream) => {
                    thread::spawn(move || {
        ...
        println!("Please provide a 3d Point as a comma separated integers");
        ...
        loop {
            ...
            stream
                ...
                .expect("Failed to write point to server");
            stream.write_all(b"\n").expect("Failed to write to server");

            let mut reader = BufReader::new(&stream);
            reader
                .read_until(b'\n', &mut buffer)
                .expect("Could not read result into buffer");

            let input = str::from_utf8(&buffer).expect("Could not write result as string");
            if input == "" {
                eprintln!("‼️ Empty response from the server");
            }
